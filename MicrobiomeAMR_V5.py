...
# Installation:
# 1.Conda: EMU, minimap2, fastp, ptrimmer, and resistomeanalyzer, Porechop, Filtlong
...
# V4 add nanopore trim informaiotn
...
AMR_parser.add_argument('-p', '--primers', help='input primers file [.txt]')
...
#EMU_parser.add_argument('-p', '--primers', help='input primers file [.txt]')
...
if args.subparser_name == "AMR":
  ...
  # if not primers:
      # print("ERROR: primers informaion not found, assign using -p")
      # exit(1)
   if args.AMR_output:
   ...
   # Change file name
   #for filename in os.listdir(AMR_query_files):
   # if filename.endswith(".fastq.gz"):
   # parts = filename.split('_')
   #  new_filename = f"{parts[0]}_{parts[3]}.fastq.gz"
   # os.rename(os.path.join(AMR_query_files, filename), os.path.join(AMR_query_files, new_filename))
  ...
  # for file in glob.glob("*_R1_trimed.fastq.gz"):
    #   f = file.replace("_R1_trimed.fastq.gz", "")
    #   subprocess.run([
       #    "ptrimmer",
        #   "-t", "pair",
       #    "-a", primers,
         #  "-f", f"{f}_R1_trimed.fastq.gz",
        #   "-r", f"{f}_R2_trimed.fastq.gz",
         #  "-d", f"{f}_R1_p_trimed.fastq",
         #  "-e", f"{f}_R2_p_trimed.fastq",
        #   "-l"
     #  ])

   # print("\n  \n")
  # print(colored('primers trimming was completed', 'red'))
 #  print('------------------------------')
 #  print("\n  \n")
...
# Change file name
   #for filename in os.listdir(EMU_query_files):
       # if filename.endswith(".fastq.gz"):
       # parts = filename.split('_')
       # new_filename = f"{parts[0]}_{parts[1]}.fastq.gz"
   #  os.rename(os.path.join(EMU_query_files, filename), os.path.join(EMU_query_files, new_filename))
         ...
         # j = file.replace("_R1_trimed.fastq.gz", "")
          #subprocess.run([
              #  "ptrimmer",
               #  "-t", "pair",
               #  "-a", primers,
                # "-f", f"{j}_R1_trimed.fastq.gz",
              #   "-r", f"{j}_R2_trimed.fastq.gz",
                # "-d", f"{j}_R1_p_trimed.fastq",
             #    "-e", f"{j}_R2_p_trimed.fastq",
              #   "-l"
              #          ])
       ...
       for file in glob.glob("*_trimed.fastq.gz"):
        ...
        #j = file.replace("_f_trimed.fastq.gz", "")
           #subprocess.run([
              ...
              # "-t", "single",
              # "-a", primers,
              # "-f", f"{j}_f_trimed.fastq.gz",
              # "-d", f"{j}_p_trimed.fastq",
              # "-l"
