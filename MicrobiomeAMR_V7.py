...
AMR_parser.add_argument('-i', '--data_input', help='input file containing sample names, fastq files paths [.txt]')
...
EMU_parser.add_argument('-i', '--data_input', help='input file containing sample names, fastq files paths [.txt]')
...
if args.subparser_name == "AMR":
   ...
   print("\n  \n")
   print(colored("Step 1: QC checks and adaptors removal", 'green'))
  ...
  # Split the line into sample and file paths using tabs
   ...
   print("\n  \n")
   print(colored("Step 2: Alignment using minimap2", 'green'))
   ...
   os.chdir('Resistomes')
   file_name_list = os.listdir('Resistomes')
   input_files_gene = [os.path.join('Resistomes', file)
               for file in file_name_list if file.endswith('.gene.tsv')]
   output_file_gene = 'AMR_combined.gene.csv'

   os.chdir('Resistomes')
   file_name_list = os.listdir('Resistomes')
   input_files_class = [os.path.join('Resistomes', file)
                  for file in file_name_list if file.endswith('.class.tsv')]
   output_file_class = 'AMR_combined.class.csv'
 ...
 # combine data for gene files
   ...
   with open(output_file_gene, 'w') as amr:
       local_sample_names = []
   ...
   with open(output_file_class, 'w') as amr:
     local_sample_names = []
     for sample, dat in samples.items():
        local_sample_names.append(sample)
     amr.write('name' + '\t' + '\t'.join(local_sample_names) + '\n')
...
# Save the results to a new CSV file
   result_gene.to_csv('AMR_gene_combined_rel-abundance.csv', sep='\t', index=False)
   result_class.to_csv('AMR_class_combined_rel-abundance.csv', sep='\t', index=False)

#Clean up: move all the temp files to a folder
   r1_amr_parent_dir = os.path.dirname(r1_amr_path)
   os.chdir(r1_amr_parent_dir)
   ...
   for f in glob.glob('*.ampcount'):
       shutil.move(f, t_output_dir)
   print("\n  \n")
   print(colored("Complete AMR genes analysis", 'green'))
   print('---------------------------')
...
if args.subparser_name == "EMU":
   ...
   print("\n  \n")
   print(colored("Step 1: QC checks and trims", 'green'))
   os.mkdir(EMU_output)
   os.chdir(EMU_output)
   if query_type == "PE":
       ...
       for line in file_name_list:
         # Split the line into sample and file paths using tabs
          ...
          subprocess.run([
               ...
               "-i", r1_16s_path,
               "-I", r2_16s_path,
               "-o", f"{sample}_R1_f_trimed.fastq.gz",
               "-O", f"{sample}_R2_f_trimed.fastq.gz",
   ...
   print("\n  \n")
   print(colored("Step 2: Characterization of bacterial abundance using emu", 'green'))

   if query_type == "PE":
       for file in glob.glob("*_R1_f_trimed.fastq"):
           j = file.replace("_R1_f_trimed.fastq", "")
           subprocess.run([
               "emu", "abundance",
               "--type", "sr",
               f"{j}_R1_f_trimed.fastq.gz",
               f"{j}_R2_f_trimed.fastq.gz",
               "--db", EMU_database,
               "--threads", threads,
               "--keep-counts",
   ...
   if query_type == "PE":
     file_paths = glob.glob("*_R1_f_trimed.fastq_rel-abundance.tsv")
   ...
   print("\n  \n")
   print(colored("Step 3: Combine tables", 'green'))
...
# Clean up: move all the temp files to a folder
   r1_16s_parent_dir = os.path.dirname(r1_16s_path)
   os.chdir(r1_16s_parent_dir)
   os.mkdir('fastp_output')
   fastp_output_dir = r1_16s_parent_dir + '/fastp_output'
   for f in glob.glob('*.sam'):
   ...
   for f in glob.glob('*.htmp'):
       shutil.move(f, fastp_output_dir)
   print("\n  \n")
   print(colored("Complete microbiome analysis", 'green'))
...
if args.subparser_name == "COR":
    ...
    print("\n  \n")
    print(colored("Step 1: obtain amr and 16s results for correlation analysis", 'green'))
    ...
    print("\n  \n")
    print(colored("Step 2: Combine bacterial and AMR genes counts", 'green'))
    ...
    print("\n  \n")
    print(colored("step 3 : P-value calculation", 'green'))
    ...
    print("\n  \n")
    print(colored("Step 4: generate correlation files and figures", 'green'))
    # remove duplications
    ...
    print("\n  \n")
    print(colored("Complete the correlation analysis", 'green'))
