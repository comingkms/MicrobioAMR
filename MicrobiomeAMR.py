...
parser.add_argument('-amr_db', '--AMR_database', help='Path to AMR database')
parser.add_argument('-emu_db', '--EMU_database', help='Path to EMU database')
parser.add_argument('-a', '--anno_file', help='AMR annotation file')
parser.add_argument('-qt', '--query_type', help='Information for PE or ONT')
parser.add_argument('-amr_q', '--AMR_query_files', help='Path to query AMR fastq files')
parser.add_argument('-emu_q', '--EMU_query_files', help='Path to query 16S fastq files')
parser.add_argument('-amr_o', '--AMR_output', help='AMR Output for AMR genes qualification')
parser.add_argument('-emu_o', '--EMU_output', help='EMU Output for microbial community ')
parser.add_argument('-cor_o', '--Cor_output', help='Correlation Output for microbial community ')
...
if args.Cor_output:
   Cor_output = args.Cor_output
    ...
    print("Skip correlation analysis")
...
os.mkdir(AMR_output_dir)
os.mkdir(EMU_output_dir)
os.mkdir(Cor_output_dir)

# QC and QA ( fastp ): remove adaptor and primers
os.chdir(AMR_query_files)
print("\n  \n")
print(colored("####################", 'yellow'))
print(colored("#", 'yellow'),colored("QC and QA checks", 'red'), colored("#", 'yellow'))
print(colored("####################", 'yellow'))
print("\n  \n")
for file in glob.glob("*_R1.fastq.gz"):
    j = file.replace("_R1.fastq.gz", "")
    subprocess.run([
        "fastp",
        "-i",  f"{j}_R1.fastq.gz",
        "-I",  f"{j}_R2.fastq.gz",
        "-o",   f"{j}_R1_trimed.fastq.gz",
        '-O',   f"{j}_R2_trimed.fastq.gz",
...
print("\n  \n")
print(colored('QC and QA checks were completed', 'red'))
print('-------------------------------')
print("\n  \n")

# SAM file using minimap2
print(colored("############################", 'yellow'))
...
for file in glob.glob("*_R1_trimed.fastq.gz"):
        j = file.replace("_R1_trimed.fastq.gz", "")
        subprocess.run(['minimap2', '-ax', 'sr', AMR_database, f'{j}_R1_trimed.fastq.gz', f'{j}_R2_trimed.fastq.gz', '-t', threads], stdout=open(f'{j}.sam', 'w'))


#elif query_type == "ONT":
    #for file in glob.glob("*_R1.fastq.gz"):
        #j = file.replace("_R1.fastq.gz", "")
        #subprocess.run(['minimap2', '-ax', 'sr', AMR_database, f'{j}_R1.fastq.gz', f'{j}_R2.fastq.gz', '-t', threads],
                       #stdout=open(f'{j}.sam', 'w'))


# os.mkdir(output)
sam_dir = os.listdir(AMR_query_files)

for sam in sam_dir:
   if sam.endswith(".sam"):
    shutil.copy(AMR_query_files + '/' + sam, AMR_output)
...
input_files = [os.path.join(AMR_output+'/Resistomes', file) for file in file_name_list if file.endswith('.tsv')]
output_file ='AMR_combined.csv'
...
result.to_csv('AMR_combined_rel-abundance.csv', sep='\t', index=False)

shutil.move('AMR_combined_rel-abundance.csv', Cor_output)

print(colored("AMR profiling (AMR_combined.csv) was saved in the Resisomes folder", 'red'))
print('------------------------------------------------------------------')
# 16s rRNA amplicon sequencing data analysis using EMU
...
if query_type == "PE":
    ...
    for file in glob.glob("*_trimed.fastq.gz"):
        j = file.replace("_trimed.fastq.gz", "")
        subprocess.run([
            "emu", "abundance",
            f"{j}_trimed.fastq.gz",
            "--db", EMU_database,
            "--threads", threads,
            "--keep-counts",
            "--min-abundance", '0.0005',
...
os.chdir(EMU_output)
os.mkdir('combined')
pattern = '*_rel-abundance-threshold-0.0005.tsv'
...
file_paths = glob.glob("*_rel-abundance-threshold-0.0005.tsv")
...
print("\n  \n")
print(colored("#####################################", 'yellow'))
print(colored("#", 'yellow'), colored("The run is completed successfully", "red"), colored('#', 'yellow'))
print(colored("#####################################", 'yellow'))
print("\n  \n")